                                                ...
                                                <td>
                                                    <div class="flex align-items-center">
                                                        <a class="btn btn-outline-info btn-sm" data-toggle="tooltip"
                                                            ...
                                                            href="{{ route('partners.edit', $user->id) }}"><i
                                                                class="far fa-edit"></i></a>
                                                        @if ($user->status == 1)
                                                            <form class="d-inline" method="post"
                                                                action="{{ route('partners.disable', $user->id) }}">
                                                                @csrf
                                                                <input type="hidden" name="status" id="status-input"
                                                                    value="0">
                                                                <button class="btn btn-outline-danger btn-sm disable-user"
                                                                    data-user-id="{{ $user->id }}" data-toggle="tooltip"
                                                        ...
                                                        @else
                                                            <form class="d-inline" method="post"
                                                                action="{{ route('partners.enable', $user->id) }}">
                                                                @csrf
                                                        ...
                                                        <a class="btn btn-outline-success btn-sm"
                                                            href="partners/{{ $user->id }}" data-toggle="tooltip"
                    ...
                    $(document).on('click', '.disable-user', function(e) {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                $.ajax({
                                    type: 'POST',
                                    url: '{{ route('partners.disable', ':id') }}'.replace(':id', userId),
                                    data: {
                                        _token: '{{ csrf_token() }}',
                                        id: userId,
                    ...
                    $(document).on('click', '.enable-user', function(e) {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                $.ajax({
                                    type: 'POST',
                                    url: '{{ route('partners.enable', ':id') }}'.replace(':id', userId),
                                    data: {
                                        _token: '{{ csrf_token() }}',
                                        id: userId,
